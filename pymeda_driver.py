        ...
        for y in range(synapse_labels.shape[1]):
            for x in range(synapse_labels.shape[2]):
                if (synapse_labels[z][y][x] in connected_components.keys()):
                    connected_components[synapse_labels[z][y][x]].append((z, y,
                                                                          x))
                else:
...
def calculate_synapse_centroids(connected_components):
    ...
    for key, value in connected_components.items():
        z, y, x = zip(*value)
        synapse_centroids.append((int(sum(z) / len(z)), int(sum(y) / len(y)),
                                  int(sum(x) / len(x))))
...
def get_aggregate_sum(synapse_centroids, data):
    ...
    for centroid in synapse_centroids:
        ...
        for key in data.keys():
            data_dictionary[key].append(
                np.sum(data[key][z_lower:z_upper, y_lower:y_upper, x_lower:
                                 x_upper]))
...
def pymeda_pipeline(predictions,
                    raw_data,
                    title="PyMeda Plots",
                    cluster_levels=2,
                    path="./"):
